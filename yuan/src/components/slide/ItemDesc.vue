    ...
    <div class="content" v-if="!props.isMy">
      <div class="location-wrapper" v-if="item.city || item.address">
        <div class="location">
          ...
          <span v-if="item.distance" class="distance">距离您{{ item.distance }}</span>
        </div>
      </div>
      <div class="live" v-if="props.isLive">直播中</div>
      <div class="name mb1r" @click.stop="$emit('goUserInfo')">
        <span class="f18 fb">{{ item?.author?.nickname }}</span>
        <span v-if="item.city || item.address" class="time">{{
          _dateFormat(item.create_time)
        }}</span>
      </div>
