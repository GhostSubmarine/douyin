...
function getInsEl(item, index, play = false) {
  // console.log('index', item, index, play)
...
function touchEnd(e) {
  ...
  slideTouchEnd(e, state, canNext, (isNext) => {
    ...
    if (props.list.length > props.virtualTotal) {
      ...
      if (userBehavior) {
        if (userBehavior[props.uniqueId]) {
          userBehavior[props.uniqueId] = {
            origin: props.uniqueId,
            num:
              state.localIndex + 1 > userBehavior[props.uniqueId].num
                ? state.localIndex + 1
                : userBehavior[props.uniqueId].num
          }
        } else {
          userBehavior[props.uniqueId] = {
            origin: props.uniqueId,
            num: state.localIndex + 1
          }
        }
        _storageSet('userBehavior', userBehavior)
      } else {
        const obj = {}
        obj[props.uniqueId] = {
          origin: props.uniqueId,
        ...
        _storageSet('userBehavior', obj)
      }

      // const userBehavior2 = _storageGet('userBehavior')
      // console.log('userBehavior: ', userBehavior2)

      if (isNext) {
        // console.log('state.localIndex: ', state.localIndex)
        if (state.localIndex + 1 >= props.list.length) {
          setTimeout(() => {
