...
import { sendUserBehavior } from '@/api/videos'
import { _storageGet } from '@/utils'
import LoginView from './components/LoaginView.vue'
...
watch(
  () => baseStore.palyTotalTime,
  async (newVal, oldVal) => {
    if (newVal === oldVal) return

    const min = Math.min(oldVal, newVal)
    const max = Math.max(oldVal, newVal)

    // 计算最大的 60 的倍数且位于 [min, max] 区间内
    const largestMultiple = Math.floor(max / 60) * 60

    // 检查是否在有效区间内
    if (largestMultiple >= min) {
      fetchIntervalData(largestMultiple)
    }
  }
)

async function fetchIntervalData(value) {
  const userBehavior = _storageGet('userBehavior')
  const nums = sumNums(userBehavior)
  await sendUserBehavior({
    vidnum: nums,
    seconds: baseStore.palyTotalTime
  })
}
...
onMounted(async () => {
  ...
  bus.on(EVENT_KEY.OPEN_COMMENTS, (value) => {
